"""
Integracao com o Melhor Envio (https://docs.melhorenvio.com.br).

Cotacao multi-transportadora + compra de etiqueta pre-paga pela
plataforma (remetente neutro). A vendedora so imprime o PDF e posta.
"""
...
def calculate(
    *, origin_cep: str, destination_cep: str, weight_grams: int,
    ...
    """POST /api/v2/me/shipment/calculate — cotacao em todas as transportadoras."""
    resp = requests.post(
...
def buy_label(
    *,
    ...
    """
    Carrinho → checkout → generate → print.

    Remetente = plataforma (nome neutro), CEP de origem = da vendedora
    (para a encomenda sair perto dela sem expor o apelido da loja).
    """
    from .services import shipping_sender

    sender = sender or shipping_sender()
    document = (sender.get("document") or "").replace(".", "").replace("/", "").replace("-", "")
    from_payload = {
        "name": sender.get("name") or settings.SITE_NAME,
        "postal_code": origin_cep,
    }
    # CNPJ (14) vs CPF (11) — ME usa campos distintos.
    if len(document) == 14:
        from_payload["company_document"] = document
    elif document:
        from_payload["document"] = document
    if sender.get("email"):
        from_payload["email"] = sender["email"]
    if sender.get("phone"):
        from_payload["phone"] = sender["phone"]
    if sender.get("address"):
        from_payload["address"] = sender["address"]
        from_payload["number"] = sender.get("number") or "S/N"
        from_payload["district"] = sender.get("district") or ""
        from_payload["city"] = sender.get("city") or ""
        from_payload["state_abbr"] = sender.get("state_abbr") or ""
...
def find_dropoff_points(*, cep: str, company_id: int | None = None) -> list[dict]:
    """Agencias/pontos de postagem proximos ao CEP da vendedora."""
